...
class PVPurchase:
    ...
    @staticmethod
    def fromPurchaseModel(purchase_model: Purchase) -> "PVPurchase | None":
        if purchase_model is None:
            return None
        data_wrapper = DatabaseObjectWrapper(jsonDict={})
        data_wrapper.set_or_append(incoming_meta_tags={
            META_TAG_KEY_VIDEO_ID: purchase_model.video_id,
            META_TAG_KEY_CAT_ID: purchase_model.cat_id
        })
        return PVPurchase(data=data_wrapper)
